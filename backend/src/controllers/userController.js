...
controllers.list_colaboradores = async (req,res) => {
  const data = await Users.findAll({
      include: [ {model: Tiposdeperfil, where: {id: 3}} ]
  })
  .then(function(data){
      return data;
  })
...
controllers.register = async (req,res) => {
    ...
    let imagem = 'https://i.ibb.co/H7TQ5Rd/download.png'
    ...
    res.status(200).json({
        success: true,
        message:"Registado com sucesso, por favor confirme o seu email",
        data: data
    });
    }
/* LOGIN ---------------------- */
  ...
  try {
    ...
    if (!user) {
      ...
      user = await Users.create({
        ...
        email: email,
        telemovel: "---", // Assign a default value or provide a valid value for the "telemovel" column
        password: hashedPassword,
        cargo: 'visitante',
        estado: 1,
        primeiro_login: 0,
...
controllers.criaruser = async (req, res) => {
  ...
  const hashedPassword = bcrypt.hashSync(password, 10);
  const picture = 'https://media.licdn.com/dms/image/C4E0BAQHQ52FVWoQA0A/company-logo_200_200/0/1537278842806?e=2147483647&v=beta&t=cKZc6CTxl6vLWiSClYj3IyUNp7UZXEWXQWVcQr5Apl0';
  const data = await Users.create({
  ...
  controllers.updateProffile = async (req, res) => {
    const id = req.params.id;
    const {primeiro_nome, ultimo_nome, telemovel, email,imagem} = req.body;
    let user = await Users.findOne({ where: { id: id } });
    if (!user) {
      res.status(403).json({
        success: false,
